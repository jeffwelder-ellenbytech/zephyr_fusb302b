 ...
 * Copyright 2024 Jeff Welder (Ellenby Technologies, Inc.)
...
static const uint8_t REG_CONTROL2 = 0x08;
...
int fusb302b_get_cc_line_source(const struct fusb302b_cfg *cfg,
  ...
  switches0 &= ~0b00001100;
  switches0 |= 0b11000100; // 0xC4
  res = i2c_reg_write_byte_dt(&cfg->i2c, REG_SWITCHES0, switches0);
  if (res != 0) {
    return -EIO;
  }
  ...
  uint8_t vbusok = status0 & (1 << 7);
  uint8_t comp = status0 & (1 << 5);
  uint8_t voltage_level_2 = status0 & 0b11;

  //   *out = CC_RES_2; // JDW TEST
  // if (vbusok) {
  // if (comp) {
  if (voltage_level_1 > voltage_level_2) {
    LOG_INF("CC pin in use: %d comp: %d", 1, comp);
    *out = CC_RES_1;
  } else if (voltage_level_2 > voltage_level_1) {
    LOG_INF("CC pin in use: %d comp: %d", 2, comp);
    *out = CC_RES_2;
...
static int get_cc_line(const struct fusb302b_cfg *cfg, enum cc_res *out) {
  ...
  /* Connect ADC to CC1 (set MEAS_CC1) */
  switches0 &= ~0b00001100;
  //   switches0 |= 0b00000100;
  switches0 |= 0b11000100; // TODO This shouldn't have PU/PD
  res = i2c_reg_write_byte_dt(&cfg->i2c, REG_SWITCHES0, switches0);
  if (res != 0) {
    return -EIO;
  }
  ...
  res = i2c_reg_read_byte_dt(&cfg->i2c, REG_SWITCHES0, &switches0);
  switches0 &= ~0b00001100;
  switches0 |= 0b11001000; // TODO this shouldn't have PU/PD
  res = i2c_reg_write_byte_dt(&cfg->i2c, REG_SWITCHES0, switches0);
  if (res != 0) {
    return -EIO;
  }
...
int fusb302b_init(const struct device *dev) {
  ...
  res = i2c_reg_write_byte_dt(&cfg->i2c, REG_CONTROL0, 0b00010000); // JDW
  // As a source this is the only interrupt I got:
  // i2c read_byte i2c@8000 22 40
...
static int fusb302b_set_cc(const struct device *dev, enum tc_cc_pull cc_pull) {
  ...
  case TC_CC_RP: {
    ...
    uint8_t switches0 = 0;
    int res = i2c_reg_read_byte_dt(&cfg->i2c, REG_SWITCHES0, &switches0);
    switches0 &= ~0b00000011; /* unset PDWN1,2 */
    switches0 |= 0b11000000;  /* set PU_EN,2 */
    res = i2c_reg_write_byte_dt(&cfg->i2c, REG_SWITCHES0, switches0);
    // return -ENOSYS;
    // JDW Jeff
    break;
  ...
  case TC_CC_OPEN: {
    ...
    // JDW removed
    // switches0 &= 0b00111100; // TODO Why was this done?
    // res = i2c_reg_write_byte_dt(&cfg->i2c, REG_SWITCHES0, switches0);
    // if (res != 0) {
    //   return -EIO;
    // }
...
static int fusb302b_set_vconn(const struct device *dev, bool enable) {
  const struct fusb302b_cfg *cfg = dev->config;
  struct fusb302b_data *data = dev->data;
  int res;
  ...
  if (enable) {
    // switches0 |= 0b00110000; // Set VCONN //TODO Reset this?
  } else {
    switches0 &= ~0b00110000; // Clear VCONN
  }
...
static int fusb302b_set_roles(const struct device *dev,
                              enum tc_power_role power_role,
                              enum tc_data_role data_role) {
  const struct fusb302b_cfg *cfg = dev->config;
  struct fusb302b_data *data = dev->data;
  LOG_INF("SET ROLES power %d, data %d", power_role, data_role);
}

static int fusb302b_set_rp_value(const struct device *dev,
                                 enum tc_rp_value rp) {
  const struct fusb302b_cfg *cfg = dev->config;
  struct fusb302b_data *data = dev->data;
  int res;
  uint8_t control0 = 0;
  res = i2c_reg_read_byte_dt(&cfg->i2c, REG_CONTROL0, &control0);
  control0 &= ~0b00001100; // Clear HOST_CUR1

  switch (rp) {
  case TC_RP_RESERVED: {
    control0 |= 0b00000000; // No Current
    res = i2c_reg_write_byte_dt(&cfg->i2c, REG_CONTROL0, control0);
  } break;

  case TC_RP_USB: {
    control0 |= 0b00000100; // Usb 80uA
    res = i2c_reg_write_byte_dt(&cfg->i2c, REG_CONTROL0, control0);
  } break;

  case TC_RP_1A5: {
    control0 |= 0b00001000; // Medium Current 180uA
    res = i2c_reg_write_byte_dt(&cfg->i2c, REG_CONTROL0, control0);
  } break;

  case TC_RP_3A0: {
    control0 |= 0b00001100; // High Current 330uA
    res = i2c_reg_write_byte_dt(&cfg->i2c, REG_CONTROL0, control0);
  } break;
...
static int fusb302b_get_cc(const struct device *dev,
  ...
  if (data->cc != 0) {
    ...
    int res = fusb302b_get_cc_line_source(cfg, &cc);
    // LOG_INF("cc: %d", cc);
    if (res != 0) {
      return -EIO;
    }
...
int fusb302b_set_cc_polarity(const struct device *dev,
  ...
  //   int res =i2c_reg_write_byte_dt(&cfg->i2c, REG_SWITCHES1, 0b00100100 |
  //   cc_select);
  uint8_t switches1 = 0;
  int res = i2c_reg_read_byte_dt(&cfg->i2c, REG_SWITCHES1, &switches1);
  switches1 &= ~0b00000011;
  // switches1 |= (1 << 4); // Data role SNK
  // switches1 |= 0b10100100; // Data role source
  switches1 |= 0b10110100; // Data role SNK
  switches1 |= cc_select;

  res = i2c_reg_write_byte_dt(&cfg->i2c, REG_SWITCHES1,
                              switches1); // TODO check data role sink
  if (res != 0) {
    return -EIO;
  }
  ...
  //   k_sleep(K_MSEC(150));
  //   k_busy_wait(150000);
  // Enable SOP // TODO Check if this is correct
  //   res = i2c_reg_write_byte_dt(&cfg->i2c, REG_CONTROL1, 0x67);
  //   if (res != 0) {
  //     return -EIO;
  //   }
...
int fusb302b_transmit_data(const struct device *dev, struct pd_msg *msg) {
  ...
  case PD_PACKET_TX_HARD_RESET:
    ...
    break;
  default:
    // LOG_ERR("Packet type %s not supported",
    // pd_packet_type_to_str(msg->type));
    res = -1;
    break;
  }
...
// https://github.com/torvalds/linux/blob/master/drivers/usb/typec/tcpm/fusb302.c
static const struct tcpc_driver_api fusb302b_tcpc_driver_api = {
    .init = fusb302_setup,
    .get_cc = fusb302b_get_cc,
    .select_rp_value = fusb302b_set_rp_value,
    .get_rp_value = NULL,
    ...
    .set_vconn = fusb302b_set_vconn,
    .set_roles = fusb302b_set_roles,
